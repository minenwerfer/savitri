...
class FileController extends controller_1.Mutable {
    constructor() {
        super(file_model_1.File, index_json_1.default);
    }
    async insert(props, res, decodedToken) {
        ...
        const oldFile = await file_model_1.File.findOne({
            $and: [
        ...
        if (oldFile) {
            ...
            try {
                ...
                await file_model_1.File.deleteOne(oldFile._id);
            }
            catch (error) {
                console.trace(error);
            }
    ...
    async remove(props) {
        const file = await file_model_1.File.findOne(props.filters);
        if (file) {
    ...
    async download(_id) {
        const file = await file_model_1.File.findOne({ _id }).lean();
        if (!file) {
